...
public class Send_Photo_Box_New extends javax.swing.JPanel {
    ...
    public void setPhoto(int ID, ImageIcon photo) {
        this.photo = photo;
        Image img;
        if (photo.getIconHeight() > 130 || photo.getIconWidth() > 130) {
            if (photo.getIconWidth() > photo.getIconHeight()) {
                img = photo.getImage().getScaledInstance(130, -1, Image.SCALE_SMOOTH);
            } else {
                img = photo.getImage().getScaledInstance(-1, 130, Image.SCALE_SMOOTH);
            }
            ImageIcon icon = new ImageIcon(img);
            lb.setIcon(icon);
        } else {
            lb.setIcon(photo);
            if (photo.getIconHeight() <= 30) {
                this.setPreferredSize(new Dimension(600, 72));
                this.setMaximumSize(new Dimension(600, 72));
            } else {
                this.setPreferredSize(new Dimension(600, photo.getIconHeight() + 37));
                this.setMaximumSize(new Dimension(600, photo.getIconHeight() + 37));
            }
        }
    ...
    private void lbMouseReleased(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_lbMouseReleased
        if (isOver) {
            SocketClientController.showImage(photo);
        }
